...
# LDFLAGS+= -L/mingw64/lib/gcc/x86_64-w64-mingw32/9.3.0/plugin -lcc1.exe
...
$(PLUGIN_DSO): $(PLUGIN_OBJECT_FILES) $(LIBGCC_C_API_SO)
	$(CC) \
	    ...
	    -shared \
	    $(PLUGIN_OBJECT_FILES) \
	    -L/mingw64/lib/gcc/x86_64-w64-mingw32/9.3.0/plugin -lcc1plus.exe  \
		-o $@ \
	    $(LIBS) \
...
$(LIBGCC_C_API_SO): $(pwd)/gcc-c-api
	cd gcc-c-api && make $(if $(srcdir),-f $(srcdir)./gcc-c-api/Makefile) libgcc-c-api.dll CC=$(CC) $(if $(srcdir),srcdir=$(srcdir)./gcc-c-api/)
